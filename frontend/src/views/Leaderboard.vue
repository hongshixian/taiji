<template>
  <div class="page-shell flex flex-col gap-8">
    ...
    <div v-for="sec in visibleSections" :key="sec" class="flex flex-col gap-5">
      <h2 class="m-0 border-b-2 border-violet-200 pb-4 text-xl font-bold text-fg dark:border-violet-500/30">{{ sec }}</h2>
      <p v-if="SECTION_DESC[sec]" class="m-0 max-w-[860px] text-sm leading-relaxed text-fg-secondary">{{ SECTION_DESC[sec] }}</p>
      ...
      <div class="overflow-hidden rounded-lg border border-line bg-surface shadow-xs">
        <div class="grid grid-cols-[52px_1fr_1fr] border-b border-line bg-surface-sunken px-6 py-3 text-xs font-semibold text-fg-secondary">
          <span>#</span>
          <span>模型</span>
          <span class="text-violet-600">综合得分（{{ activeBenchName(sec) }}）</span>
        </div>
        <div
...
const BENCHMARK_DESC: Record<string, string> = {
