...
- **Superuser** (`users.is_superuser`): platform operators who can cross tenant boundaries. JWT carries `is_superuser` claim; when true, `g.bypass_tenant_filter=True` is set automatically by the before_request hook, exposing all tenants' data through normal endpoints. The dedicated `/api/v1/superadmin/*` blueprint is gated by `@superuser_required` and manages tenants themselves. The first admin seeded by the entrypoint always gets `is_superuser=true`.
...
- **New business model**: extend `TenantMixin` (`from app.models._tenant_mixin import TenantMixin`) so it gets `tenant_id` and is included in the auto-filter. Otherwise data leaks across tenants.
...
- **Frontend permission gates**: use `const { has } = usePermission()` from `composables/usePermission.js`; `v-if="has('user:write')"` on UI elements. Router guards in `router/index.js` use `meta.requiresPermission`.
