...
class TestTenantIsolation:
    ...
    def test_user_only_sees_own_tenant_tasks(self, client):
        """租户 A 用户只能看到 A 租户下的任务"""
        resp = client.get("/api/v1/tasks/benchmark/",
                          headers={"Authorization": f"Bearer {self.token_a}"})
        assert resp.status_code == 200
        names = [i["task_name"] for i in resp.get_json()["data"]["items"]]
        assert "A 的测评" in names
        assert "B 的测评" not in names

        resp = client.get("/api/v1/tasks/benchmark/",
                          headers={"Authorization": f"Bearer {self.token_b}"})
        names = [i["task_name"] for i in resp.get_json()["data"]["items"]]
        assert "B 的测评" in names
        assert "A 的测评" not in names
    ...
    def test_cannot_access_other_tenant_task_by_id(self, client):
        ...
        resp = client.get(f"/api/v1/tasks/benchmark/{b_task_id}",
                          headers={"Authorization": f"Bearer {self.token_a}"})
        assert resp.status_code == 404
        assert resp.get_json()["code"] == 20001
    ...
    def test_superuser_can_manage_tenant_members(self, client, app):
        ...
        resp = client.post("/api/v1/superadmin/tenants/10/members",
                           headers={"Authorization": f"Bearer {token}"},
                           json={"identifier": "tenant-added", "role": "user"})
        assert resp.status_code == 200
        ...
        resp = client.post("/api/v1/superadmin/tenants/10/members",
                           headers={"Authorization": f"Bearer {token}"},
                           json={"identifier": "not-registered-user", "role": "user"})
        assert resp.status_code == 404
