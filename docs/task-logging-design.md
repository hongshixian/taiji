...
任务日志围绕具体任务查看，不单独建设任务日志中心。

在每个任务类型的任务列表中，日志入口放在任务状态列之后、行操作列之前：
...
- 任务日志用于解释任务状态变化和执行过程，和状态列强相关。
- 日志查看属于信息查看，不是重试 / 删除这类行为操作，放在操作列左侧更自然。
- 用户通常从某个任务出发查看日志，不需要先进入独立日志中心。

按钮展示规则：

- 建议所有任务行都展示“日志”按钮，保持表格布局稳定。
- 如果 `log_path` 存在，点击后读取并展示 JSONL 日志。
- 如果 `log_path` 为空或日志文件不存在，弹窗 / 抽屉展示“暂无执行日志”。
- 任务类型未接入任务日志服务时，也可以保留按钮并展示空态。

后续实现时，网页分析任务页和 CSV 检查任务页都应按这个列顺序接入。
