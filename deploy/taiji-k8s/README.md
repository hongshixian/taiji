...
- PostgreSQL、Redis、NATS 使用 `rancher-local-path` PVC，仅适合当前单节点绑定部署；
...
自定义镜像默认使用 `b059d56` 标签。发布新的应用代码时，通过 `IMAGE_TAG` 指定新标签，
并同步更新清单中的四个自定义镜像标签后再部署。

部署命令会从 Docker `.env` 幂等创建 `taiji-secrets`，然后统一执行
`kubectl apply -k deploy/taiji-k8s`。IAM Bootstrap Job 每次发布前会删除并重建，
以便幂等校正 Realm 配置。
...
kubectl -n lihao port-forward service/iam-proxy 28180:8080
curl http://127.0.0.1:28180/realms/fangcun/taiji-iam/health
```
